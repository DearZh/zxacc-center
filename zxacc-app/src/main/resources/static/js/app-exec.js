window.AppExec = {
	pageInit: function (event, page) {
		...
		axios.post(__ctx + '/index/loadByPaper', 'id=' + paperId)
...
//			$$('.zx-cards').html(cardsCompile(response.data));
		...
		$$('.zx-cards').on('click', '.button', function(){
			...
			axios.post(__ctx + '/index/loadQues', 'id=' + quesId + '&paperId=' + paperId)
			.then(function (response) {
		...
		$$('.zx-ans').on('click', '.item-content', function(){
			...
			window.setTimeout(function(){
				...
				app.request.postJSON(__ctx + '/index/saveExec', param, function(data){
		...
		$$('.zx-submit').click(function(){
			...
			app.dialog.confirm('确定提交吗', function(index){
				...
				app.request.postJSON(__ctx + '/index/submitExec', params, function(data){
					var taskId = data.id;
					app.dialog.alert('提交成功', function(){
						location.href = __ctx + '/read?taskId=' + taskId;
					});
				});
			});
		});
