...
var app = new Framework7({
	...
	routes: [
	     {
	    	 ...
	    	 async(routeTo, routeFrom, resolve, reject) {
	    		 ...
	    		 if (status==1) {
	    			 //考试完毕，则进入只读页面
	    			 resolve({ url: __ctx + '/read?taskId=={{taskid}}' })
	    		 } else {
	    			 //进入考试页面
	    			 app.dialog.confirm('倒计时即将开始，确认进入考试吗', function(){
	    				 resolve({ url: __ctx + '/exec?id={{id}}' })
	    			 })
	    		 }
	    	 },
	    	 on: {
	...
	on: {
		init: function(){
			...
			axios.post(__ctx + '/index/loadTaskList')
			...
			axios.post(__ctx + '/index/loadUser')
			.then(function (response) {
